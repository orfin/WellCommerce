 ...
 */
namespace WellCommerce\ShippingMethod\DependencyInjection\Compiler;
 ...
 *
 * @package WellCommerce\ShippingMethod\DependencyInjection\Compiler
...
class RegisterShippingCalculatorsPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds('shipping_method.calculator') as $id => $attributes) {
            $calculator = $container->getDefinition($id);
            $class        = $calculator->getClass();
            $refClass     = new \ReflectionClass($class);
            $interface    = 'WellCommerce\\ShippingMethod\\Calculator\\ShippingMethodCalculatorInterface';

            $calculator->setProperty('alias', $attributes[0]['alias']);
