 ...
 */
namespace WellCommerce\PaymentMethod\DependencyInjection\Compiler;
 ...
 *
 * @package WellCommerce\PaymentMethod\DependencyInjection\Compiler
...
class RegisterPaymentMethodPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds('payment_method.processor') as $id => $attributes) {
            ...
            $interface = 'WellCommerce\\PaymentMethod\\Processor\\PaymentMethodProcessorInterface';
        ...
        foreach ($container->findTaggedServiceIds('payment_method.configurator') as $id => $attributes) {
            $class     = $container->getDefinition($id)->getClass();
            $refClass  = new \ReflectionClass($class);
            $interface = 'WellCommerce\\PaymentMethod\\Configurator\\PaymentMethodConfiguratorInterface';
            if (!$refClass->implementsInterface($interface)) {
                throw new \InvalidArgumentException(sprintf('Service "%s" must implement interface "%s".', $id, $interface));
            }
            $definition->addMethodCall('add', array(
